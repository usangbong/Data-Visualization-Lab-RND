...
class App(QDialog):
    ...
    def createCtrlLayout(self):
        ...
        self.button_learning_VGG16 = QPushButton('learning_VGG16')
        self.button_learning_LeNet = QPushButton('learning_LeNet')
        self.button_learning_AlexNet = QPushButton('learning_AlexNet')
        #self.button_classify = QPushButton('classify result')

        layout.addWidget(self.button_cnn_model_load, 0,0)
        layout.addWidget(self.button_learning_VGG16, 0,2)
        layout.addWidget(self.button_learning_LeNet, 0,3)
        layout.addWidget(self.button_learning_AlexNet, 0,4)
        ...
        self.button_learning_VGG16.clicked.connect(self.VGG16LearningButton)
        self.button_learning_LeNet.clicked.connect(self.LeNetLearningButton)
        #self.button_classify.clicked.connect(self.on_classyfyButton_clicked)
        self.button_learning_AlexNet.clicked.connect(self.AlexNetLearningButton)
    ...
    def LoadCNNModel(self) :
        ...
        _modelFileName = self.modelFileName
        _modelFileName = _modelFileName.split('/')
        _modelFileName = _modelFileName[len(_modelFileName)-1]
        print(_modelFileName)
        _modelFileName = _modelFileName.split('_')
        self.modelFileSize = int(_modelFileName[0])

        print("model img size :" + str(self.modelFileSize))
    ...
    def VGG16LearningButton(self) :
        from learning import Learning
        #deep learning model reversion
        Learning.learning_model_VGG16()
        print('learning model END')

    def LeNetLearningButton(self) :
        from learning import Learning
        Learning.learning_model_LeNET()
        print('learning model END')

    def AlexNetLearningButton(self) :
        from learning import Learning
        Learning.learning_model_AlexNet()
        print('learning model END')
    ...
    def imgToUI(self) :
        ...
        if self.semiAutoBox.isChecked() :
            ...
            for i in self.subImglist :
                ...
                if self.modelFileSize is 32 :
                    i = cv2.cvtColor(i, cv2.COLOR_BGR2GRAY)
                _classnum = Predict.predict_classnum(i, _model, self.modelFileSize)
