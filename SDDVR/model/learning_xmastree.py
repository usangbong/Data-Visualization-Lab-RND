...
class Learning():
    def create_learning_model_VGG16(classes) :
        #model = Learning.VGG16model(classes)
        #model = Learning.smallerVGGmodel(classes)
        #model = Learning.RESmodel(classes)
        model = Learning.RESmodel2(classes)
        return model

    def create_learning_model_RES(classes) :
        model = Learning.RESmodel(classes)
        return model
...
#    def AlexnetModel(img_shape=(224, 224, 3), n_classes=5, l2_reg=0., weights=None):
    ...
    def RESmodel2(classes) :
        input_tensor = Input(shape=(64, 64, 3), dtype='float32', name='input')
        x = Conv2D(64, (7, 7), padding='same')(input_tensor)
        ...
        x = Conv2D(32, (1, 1), padding='same')(x)
        x = BatchNormalization()(x)
        shortcut = Conv2D(32, (1, 1), padding='same')(shortcut) 
        x = Add()([x, shortcut])
        x = Activation('relu')(x)
        x = MaxPooling2D((3,3),2)(x)
        #2nd block
    ...
    def RESmodel(classes) :
        input_tensor = Input(shape=(64, 64, 3), dtype='float32', name='input')
        x = MaxPooling2D((3,3),2)(input_tensor)
        #1st block
        shortcut = x
        x = Conv2D(32, (1, 1), padding='same')(x)
        x = BatchNormalization()(x)
   ...
   # def parameters..(dataset) :
    ...
    def _setTrainData() :
        ...
        _trainset = _traingen.flow_from_directory('../imgs/xmastree/classified/img_type2_train', target_size = (64,64),
    ...
    def _setTestData() :
        ...
        _testset  = _testgen.flow_from_directory('../imgs/xmastree/classified/img_type2_test', target_size = (64,64),
                                                 batch_size = 64, color_mode='rgb', class_mode = 'categorical',
                                                 shuffle = True)
        return _testset
    ...
    def _setTrainData_gray() :
        _traingen = ImageDataGenerator(rescale=1./255, shear_range=0.2, zoom_range=0.2, horizontal_flip=True)
        _trainset = _traingen.flow_from_directory('../imgs/xmastree/classified_gray/img_type2_train', target_size = (64,64),
                                                 batch_size = 64, color_mode='rgb', class_mode = 'categorical',
                                                 shuffle = True)
        return _trainset
    ...
    def _setTestData_gray() :
        _testgen = ImageDataGenerator(rescale=1./255, shear_range=0.2, zoom_range=0.2, horizontal_flip=True)
        _testset  = _testgen.flow_from_directory('../imgs/xmastree/classified_gray/img_type2_test', target_size = (64,64),
                                                 batch_size = 64, color_mode='rgb', class_mode = 'categorical',
                                                 shuffle = True)
        return _testset
    ...
    def _setTrainData_edge() :
        _traingen = ImageDataGenerator(rescale=1./255, shear_range=0.2, zoom_range=0.2, horizontal_flip=True)
        _trainset = _traingen.flow_from_directory('../imgs/xmastree/classified_edge/img_type2_train', target_size = (64,64),
                                                 batch_size = 64, color_mode='rgb', class_mode = 'categorical',
                                                 shuffle = True)
        return _trainset
    ...
    def _setTestData_edge() :
        _testgen = ImageDataGenerator(rescale=1./255, shear_range=0.2, zoom_range=0.2, horizontal_flip=True)
        _testset  = _testgen.flow_from_directory('../imgs/xmastree/classified_edge/img_type2_test', target_size = (64,64),
                                                 batch_size = 64, color_mode='rgb', class_mode = 'categorical',
                                                 shuffle = True)
        return _testset
    ...
    def _setTrainData_LeNET() :
        _traingen = ImageDataGenerator(rescale=1./255, shear_range=0.2, zoom_range=0.2, horizontal_flip=True)
        _trainset = _traingen.flow_from_directory('../imgs/xmastree/classified/img_type2_train', target_size = (32,32),
    ...
    def _setTestData_LeNET() :
        _testgen = ImageDataGenerator(rescale=1./255, shear_range=0.2, zoom_range=0.2, horizontal_flip=True)
        _testset  = _testgen.flow_from_directory('../imgs/xmastree/classified/img_type2_test', target_size = (32,32),
                                                 batch_size = 32, color_mode='grayscale', class_mode = 'categorical',
                                                 shuffle = True)
        return _testset
    ...
    def splitData() :
        for i in range(1, 4) :
            file_list = os.listdir('../imgs/xmastree/classified/img_type2_check/%s' %(i))
            file_list.sort()
            ...
            for filename in (file_list) :
                ...
                if  rnum < 0.7 :
                    ...
                    shutil.move('../imgs/xmastree/classified/img_type2_check/%s/%s' %(i, filename),
                                '../imgs/xmastree/classified/img_type2_train/%s' %(i))
                else :
                    #print('move to test %s' % (filename))
                    shutil.move('../imgs/xmastree/classified/img_type2_check/%s/%s' %(i, filename),
                                '../imgs/xmastree/classified/img_type2_test/%s' %(i))
    ...
    def canny_edge_learning() :
        ...
        path1 = '../imgs/xmastree/classified'
        path2 = '../imgs/xmastree/classified_edge'
    ...
    def gray_learning() :
        ...
        path1 = '../imgs/xmastree/classified'
        path2 = '../imgs/xmastree/classified_gray'
        Learning.create_empty_dirtree(path1, path2)
    ...
    def learning_model_VGG16() :
        mlb = Learning.get_mlb()
        model = Learning.create_learning_model_VGG16(len(mlb.classes_))
        ...
        cnt = 0
        for root, dirs, files in os.walk('../imgs/xmastree/classified'):
            cnt += len(files)
        print('found %d files' %(cnt))
