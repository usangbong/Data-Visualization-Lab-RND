...
namespace RenderHeads.Media.AVProMovieCapture
{
	...
	public class CaptureBase : MonoBehaviour
	{
		...
		public void SelectCodec(bool listCodecs)
		{
...
#else
			_codecIndex = 0;
			if (_forceVideoCodecIndex > _codecIndex)
			{
				// The user has specified their own codec index
				if (_forceVideoCodecIndex < numVideoCodecs)
				{
			...
			_codecName = NativePlugin.GetAVIVideoCodecName(0);
#endif
		}
		...
		protected void GenerateFilename()
		{
			...
			{
				filename = _filenamePrefix + "/" + _filenamePrefix + string.Format("-%0{0}d.{1}", _imageSequenceZeroDigits, Utils.GetImageFileExtension(_imageSequenceFormat));
			}
