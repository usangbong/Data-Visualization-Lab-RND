...
public class ConflictColliderGm : MonoBehaviour
{
    ...
    private void Update()
    {
        if (col && !timeBool)
        {
            gm.transform.Translate(pos * 5.0f * Time.deltaTime);
        }
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            pos = diff(gm.transform.position, other.gameObject.transform.position);
            col = true;
