...
public class LaserPoint : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            hitPoint = hit.point;
            if (hit.transform.tag == "Controller" || hit.transform.tag == "Column") laser.SetActive(false);
            else ShowLaser(hit);
            ...
            if(hit.transform.tag == "Reset")
            {
                if(teleportAction.GetStateDown(handType))
                {
                    ...
                    GameObject tmp = SelectObject;
                    InitializeObject();
                    SelectObject = tmp;
                    share.SelectObject = tmp;
                }
            }
            ...
            {
                if(teleportAction.GetStateDown(handType)) Stop();
            }
        }
        ...
        if (SelectObject != null && SelectBluePrint != null && !SelectObject.activeSelf && SelectBluePrint.activeSelf)
        {
    ...
    GameObject getBluePrint(GameObject select)
    {
        ...
        setObjectTransform(blueprint, select.transform.position, select.transform.rotation.eulerAngles);
    ...
    void Stop()
    {
        ...
        for (var i = 0; i < parent.transform.childCount; i++)
        {
            if (parent.transform.GetChild(i).gameObject == SelectObject)
            {
                setObjectTransform(SelectObject, initPos[i], initRot[i].eulerAngles);
                playList[i] = false;
                break;
            }
        }
    ...
    void setObjectTransform(GameObject obj, Vector3 pos, Vector3 rot)
    {
    ...
    IEnumerator playObject(GameObject obj, int idx)
    {
        ...
        setObjectTransform(obj, pos[0], rot[0]);

        var j = 0;

        while(playList[idx])
        {
            setObjectTransform(obj, pos[j], rot[j]);
