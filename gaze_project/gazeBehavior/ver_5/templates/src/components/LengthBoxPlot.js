...
function LengthBoxPlot(props) {
  const { width, height, getData } = props;
  const svgRef = useRef();
  const d3 = window.d3;
  ...
  useEffect(() => {
    ...
    var margin = {top: 30, right: 50, bottom: 30, left: 15};
    ...
    for(let i=0; i<getData.length; i++){
      ...
      for(let j=0; j<getData[i].length; j++){
        if(getData[i][j]["length"] == 0){
          continue;
        }
        groupCounts[key].push(getData[i][j]["length"]);
        globalCounts.push(getData[i][j]["length"]);
      }
    }
