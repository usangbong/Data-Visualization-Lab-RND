...
def get_adjacency_matrix(distance_df, sensor_ids, normalized_k=0.1):
    ...
    df_adj_mx.to_csv("../data/distances_mx.csv",mode='w',index=True)
    
    return sensor_ids, sensor_id_to_ind, adj_mx


def get_adjacency_matrix_origin(distance_df, sensor_ids, normalized_k=0.1):
    num_sensors = len(sensor_ids)
    dist_mx = np.zeros((num_sensors, num_sensors), dtype=np.float32)
    dist_mx[:] = np.inf
    # Builds sensor id to index map.
    ...
    adj_mx[adj_mx < normalized_k] = 0
    adj_mx[adj_mx==float('inf')] = 0

    # Save adj_mx
    df_adj_mx = pd.DataFrame(adj_mx, columns=sensor_ids, index=sensor_ids)
    print("df_adj_mx : ", df_adj_mx.shape)
    print(df_adj_mx)
    
    df_adj_mx.to_csv("../data/distances_mx_origin.csv",mode='w',index=True)
    return sensor_ids, sensor_id_to_ind, adj_mx
