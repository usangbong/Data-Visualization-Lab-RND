...
class MarvelmindHedge (Thread):
    def __init__ (self, adr=None, tty="COM1", baud=9600, maxvaluescount=3, debug=False, recieveUltrasoundPositionCallback=None, recieveImuRawDataCallback=None, recieveImuDataCallback=None, recieveUltrasoundRawDataCallback=None):
        self.tty = tty  # serial
    ...
    def position_str(self):
        return "Hedge {:d}: X: {:.3f}, Y: {:.3f}, Z: {:.3f}, Angle: {:d} at time T: {:.2f}".format(self.position()[0], self.position()[1], self.position()[2], self.position()[3], self.position()[4], self.position()[5]/1000.0)
    ...
    def write_log(self):
        print(self.log)
        f = open("marvelmind log " + datetime.now().strftime('%Y-%m-%d %H %M %S') + ".txt", "w")
        for log in self.log:
            f.write("Hedge {:d}: X: {:.3f}, Y: {:.3f}, Z: {:.3f}, Angle: {:d} at time T: {:.2f}\n".format(log[0], log[1], log[2], log[3], log[4], log[5]/1000.0))
        f.close();
