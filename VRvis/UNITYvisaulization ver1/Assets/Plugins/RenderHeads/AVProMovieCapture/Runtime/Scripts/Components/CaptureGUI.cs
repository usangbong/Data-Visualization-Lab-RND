	...
	public class CaptureGUI : MonoBehaviour
	{
		...
		private void CreateGUI()
		{
			...
			if (numVideoCodecs > 0)
			{
				...
				for (int i = 0; i < numVideoCodecs; i++)
				{
					_videoCodecNames[i] = NativePlugin.GetAVIVideoCodecName(i);
					_videoCodecConfigurable[i] = NativePlugin.IsConfigureVideoCodecSupported(i);
				}
#endif
			}
			...
			if (numAudioCodecs > 0)
			{
				...
				for (int i = 0; i < numAudioCodecs; i++)
				{
					_audioCodecNames[i] = NativePlugin.GetAVIAudioCodecName(i);
					_audioCodecConfigurable[i] = NativePlugin.IsConfigureAudioCodecSupported(i);
				}
			}
#endif
		...
		private void MyWindow(int id)
		{
			...
			if (_movieCapture != null)
			{
				...
				CaptureBase.OutputType outputType = (CaptureBase.OutputType)GUILayout.SelectionGrid((int)_movieCapture._outputType, _outputType, _outputType.Length);
				if (outputType != _movieCapture._outputType)
				{
					_movieCapture._outputType = outputType;
					// TODO: Set this to last used or sensible platform default
					switch (outputType) {
						case CaptureBase.OutputType.VideoFile:
							break;
						case CaptureBase.OutputType.ImageSequence:
							break;
						case CaptureBase.OutputType.NamedPipe:
							break;
					}
				}
				GUILayout.EndHorizontal();
				...
				if (_movieCapture._outputType == CaptureBase.OutputType.VideoFile)
				{
					...
					if (_videoCodecNames != null && _shownSection == 0)
					{
						...
						int newCodecIndex = GUILayout.SelectionGrid(-1, _videoCodecNames, 1);
#if UNITY_EDITOR_WIN || (!UNITY_EDITOR && UNITY_STANDALONE_WIN)
						newCodecIndex -= 2;
#endif
...
#else
								_movieCapture._codecName = _videoCodecNames[_movieCapture._codecIndex];
#endif
					...
					if (_audioCodecNames != null && _shownSection == 2)
					{
...
#if UNITY_EDITOR_WIN || (!UNITY_EDITOR && UNITY_STANDALONE_WIN)
						int newCodecIndex = GUILayout.SelectionGrid(-1, _audioCodecNames, 1);
						GUILayout.EndScrollView();
						newCodecIndex -= 1;
						if (newCodecIndex >= -1)
						{
						...
						{
							_movieCapture._audioCodecIndex = newCodecIndex;
							_movieCapture._audioCodecName = _audioCodecNames[_movieCapture._audioCodecIndex];
							_shownSection = -1;
						}
#endif
				...
				if (_movieCapture._outputType == CaptureBase.OutputType.VideoFile)
				{
					_movieCapture._allowManualFileExtension = false;
					GUILayout.Label("Filename Prefix: ");
					_movieCapture._filenamePrefix = GUILayout.TextField(_movieCapture._filenamePrefix, 64);
			}
				else if (_movieCapture._outputType == CaptureBase.OutputType.ImageSequence)
				{
					GUILayout.Label("Filename Prefix: ");
				...
				else if (_movieCapture._outputType == CaptureBase.OutputType.NamedPipe)
				{
					...
					_movieCapture._namedPipePath = GUILayout.TextField(_movieCapture._namedPipePath, 64);
				}
		...
		private void GUI_RecordingStatus()
		{
			...
			if (_movieCapture.FPS > 0f)
			{
				...
				{
					float fpsDelta = (_movieCapture.FPS - _movieCapture.FrameRate);
					GUI.color = Color.red;
					if (fpsDelta > -10)
					{
				...
				DrawGuiField("Capture Rate", string.Format("{0:0.##} / {1:F2} FPS", _movieCapture.FPS, _movieCapture.FrameRate));
		...
		private void Update()
		{
			if (_movieCapture != null)
			{
				...
				if (_movieCapture.IsCapturing())
				{
					...
					if (!_movieCapture._isRealTime)
					{
						_lastEncodedSeconds = (uint)Mathf.FloorToInt((float)_movieCapture.NumEncodedFrames / _movieCapture.FrameRate);
					}
					else
					{
					...
					_lastEncodedFrame = _movieCapture.NumEncodedFrames % (uint)_movieCapture.FrameRate;
				}
			}
		}
	}
