...
# hbyeon 수정
# define dataframe
df_data = [['v1',1,1,1],['v2',1,1,1],['v3',1,1,1]]
df_vehicle_record = pd.DataFrame(df_data, columns=['vehicle_id','frame_count','xpos','ypos'])
#print('Original DataFrame\n------------------')
#print(df_vehicle_record)
######################################
...
class CounterThread(QThread):
                ...
                if self.running_flag:
                    if not self.pause_flag:
                        ...
                        if ret:
                            if frame_count % 6 == 0:
                                a1 = time.time()
                                frame = self.counter(self.permission, self.colorDict, frame,np.array(self.countArea), self.mot_tracker, video, frame_count)
    ...
    def counter(self, permission, colorDict, frame, CountArea, mot_tracker, videoName, frame_count):
        ...
        for i, item in enumerate(track_bbs_ids):
            ...
            cv2.putText(frame, str(id) + "-" + objectName, (x1 - 1, y1 - 3), cv2.FONT_HERSHEY_COMPLEX, 0.7, boxColor, thickness=1)

            pos_x = 0
            pos_y = 0
            pos_x = (x1+x2)/2
            pos_y = (y1+y2)/2
            #print("vehicle id", objectName + "_" + str(id), "  position_x:",pos_x,"  position_y:",pos_y)
            
            
            

            #print('updated DataFrame\n------------------')
            global df_vehicle_record
            #print("=============")
            #print(frame_count)
            new_row = {'vehicle_id':objectName + "_" + str(id), 'frame_count':frame_count, 'xpos':pos_x, 'ypos':pos_y}
            df_vehicle_record = df_vehicle_record.append(new_row, ignore_index=True)            
            #print(df_vehicle_record)
        ...
        df_vehicle_record.to_csv('vehicle_traj.csv', sep=',', encoding='utf-8')
        return frame
