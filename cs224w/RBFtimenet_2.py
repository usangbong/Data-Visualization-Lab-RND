...
class RBFtimenet_2d(object):
    # This is one layer RBFnn, two-dims
    ...
    def fit(self, X, y, target = 0):
        self.target = target
        self.c, self.s = self.cluster(X)
        self.w_x1 = np.random.randn(len(X), self.timelag)
        self.w_x2 = np.random.randn(len(X), self.timelag)
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(len(X)):
                rbf_x1 = np.array([self.rbf(x[0], self.c[i][0], self.s[i][0]) for x in X[i]])
                rbf_x2 = np.array([self.rbf(x[1], self.c[i][1], self.s[i][1]) for x in X[i]])

                F = rbf_x1.T.dot(self.w_x1[i]) + rbf_x2.T.dot(self.w_x2[i])  + self.b[i]
                loss = -(y[i][se] - F)
                ...
                self.w_x1[i] += self.lr * rbf_x1.reshape(10,) * loss
                self.w_x2[i] += self.lr * rbf_x2.reshape(10, ) * loss
